...
export const useDateRange = ({max, min, value, renderLabel, soFar, precision}) => {
    ...
    const length = precisionToLength(precision);
    const isSofar = value === 'sofar';
    return useMemo(() => {
        return isSofar ? [yearList, [], [], [], []].slice(0, length) : [yearList, monthList, dayList, hourList, minuteList].slice(0, length);
    }, [isSofar, length, yearList, monthList, dayList, hourList, minuteList]);
};
